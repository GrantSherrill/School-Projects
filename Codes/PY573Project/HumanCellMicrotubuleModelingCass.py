...
#Initializing Biological constants from Cassimeris 2018
...
pts=50 #grid point dim
Conc=35 #tubulin in uM 
knuc1=0.0005*10/50 #Nucleation Parameter
DC=1.1*10/pts*10/pts*0.1 #Diffusion constant in um^2/s
scalefac=1/Conc #amount of tubulin in polymerization
#Note paper I'm getting constants from assumed that the growth rate and shortening rate are dependent on the tubulin, 
#but this would cause issues with my finite difference model. Plus they are both about the same at 0.192 and 0.218 respectively
kcat=0.026*10/50 #Catastrophe in 1/s
kres=0.175*10/50 #Rescue in 1/s
...
#Make DiffusionGrid
Dgrid=np.ones((L*pts,L*pts,T))*Tubulinstart
...
for i in range(0,T-1): #Walking through all time steps
        ...
        for n in range(0,pts):
            ...
            if np.abs((m-pts/2)**2/n1**2)+np.abs((n-pts/2-xshift)**2/n2**2)<1 or np.abs((m-pts/2)**2/n1**2)+np.abs((n-pts/2+xshift)**2/n2**2)<1:
                if tempgrid[m,n,i]>=1: #If we are occupied, we skip the nucleation step
                    ...
                    knucint=Diffgrid[m,n,i]*knuc1/Tubulinstart
                    if 1-np.exp(-knucint)>=np.random.random():
                        MTs["MT{0}".format(grows)]=np.array([[[m,n],1]])#Add coordinate to list of coordinates where MT has been
                        grows=grows+1
    for q in range(len(MTs)): # Go through each MT, let it grow via random walk in biased direction
        ...
        if len(mt)==0: #If empty, skip. We do not delete dict entires
            ...
            if state==1: #Note we want to restrict movement inside of cell, so if it would walk out, don't. 
                ...
                #kcatint=kcat
                kcatint=(1-Diffgrid[x,y,i])*kcat #Setting variable catasrophe rate
                if 1-np.exp(-kcatint)>np.random.random():#Check for catasrophe after walking
                    if mt.ndim==2:
            ...
            if state==2: # if shirnking
                ...
                kresint=Diffgrid[x,y,i]*kres #adjust rescue rate
                if 1-np.exp(-kresint)>np.random.random(): #Check for rescue
                    if mt.ndim==2:
                        for j in range(len(mt)):
                            mt[j][1]=1#Set all of the states of the MTs to grow (1)
                    else:
                        mt[1]=1
        MTs["MT{0}".format(q)]=mt #Update Dict entry to each MT
    ...
    counter=0 #variables and subroutine to create the length and MT num graphs
    lens=[]
...
#This is because we choose D[i+1]=D[i].
        ...
        if state==1:
            ...
            #Diffgrid[x,y,i+1]=Diffgrid[x,y,i]+Tubulinstart
...
plt.yticks(np.arange(0, max(numMt), step=2))
#plt.savefig('Num2.png',dpi=600)
...
#plt.savefig('Lengths2.png',dpi=600)
...
#Plotting and making a movie of the Diffusion over time
