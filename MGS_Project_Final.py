# -*- coding: utf-8 -*-
"""
Created on Thu May 11 11:11:32 2023
...
#Flip can be removed if data does not wish tobe cropped. Check on small subset to see if flip is working correctly
...
#%%For Comparing error of evaluation to training
...
#%%Plotting predicted and real data, n chooses specific data point to look at
n=48
for i in range(5):
...
thresh=threshold_otsu(PredictArray[56,:,:,0])
binary=PredictArray[56,:,:,0]>thresh
plt.imshow(binary,cmap='Greys')
#%%Running network on unseen data
...
#Load training images
#Target here refers to the new input data
c=44
TargetArray=np.zeros([nImage,236,32,1],dtype=float)
TargetCrop=np.zeros([nImage,236-c,32,1],dtype=float)
...
#%%Plotting the original data and the prediciotn beside each other
i=0
n=401
for i in range(1):
